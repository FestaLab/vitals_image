...
module VitalsImage
  class Engine < ::Rails::Engine
    ...
    config.vitals_image.transformations                 = {}
    ...
    initializer "vitals_image.transformations" do
      config.after_initialize do |app|
        VitalsImage.transformations = {
          jpeg: {
            vips:        { saver: { strip: true, quality: 80, interlace: true, optimize_scans: true, optimize_coding: true, trellis_quant: true, quant_table: 3, overshoot_deringing: true } },
            mini_magick: { saver: { strip: true, quality: 80, interlace: "JPEG", sampling_factor: "4:2:0", colorspace: "sRGB" } }
          },
          png: {
            vips:        { saver: { strip: true, compression: 6 } },
            mini_magick: { saver: { strip: true, quality: 75 } }
          },
          webp: {
            vips:        { saver: { strip: true, quality: 75, lossless: false, alpha_q: 85, reduction_effort: 6, smart_subsample: true } },
            mini_magick: { saver: { strip: true, quality: 75, define: { webp: { lossless: false, alpha_quality: 85, thread_level: 1 } } } }
          }
        }.deep_merge app.config.vitals_image.transformations
      end
    end
